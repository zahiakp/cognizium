...
export async function POST(request) {
  ...
  try {
    ...
    const programSql = `SELECT * FROM programs WHERE status='announced' AND isGroup=0`;
    ...
    const results = await Promise.all(
      await programData.map(async (program) => {
        ...
        const [programListData, programListFields] = await connection.query(
          programListSql
        );
    ...
    const categoryData = await Promise.all(
      categories.map(async (cat) => {
        const studentsWithPoints = await Promise.all(
          studentsData
            .filter((student) => (student.category==cat.id))
            .map(async (student) => {
              let studentPoints = 0;
              ...
              await Promise.all(
                results
                  ...
                  .map(async (result) => {
                    studentPoints +=
                      (result.first
                        ?.filter((item) => item.student === student.jamiaNo)
              ...
              return { ...student, points: studentPoints,studentName:studentsData.find(item=>item.jamiaNo==student.jamiaNo).name, campusName:campusData.find(item=>item.jamiaNo==student.campus).name};
            })
        );

        return {
          category: cat,
          students: studentsWithPoints.sort((a, b) => b.points - a.points).slice(0,10),
